...
<%!
	//선언부영역: 멤버변수와 멤버메서드를 정의하는 영역이다 
	String url="jdbc:mariadb://localhost:3306/db1202";
	String user="root";
	String password="1234";
%>
<%
//클라이언트가 전송한 파라미터를 받아서 mysql에 넣을 예정이므로, 별도의 디자인 코드는 필요하지 않다.! 
out.print("이페이지에서 클라이언트가 전송한 파라미터들을, 데이터베이스에 넣을예정");

//파라미터가 영문이 아닌경우 깨진다. 따라서 파라미터를 대상으로 한 인코딩을 원하는 언어로 지정하면된다. 
//주의! 파라미터를 받기전에 미리 세팅해야한다. 
request.setCharacterEncoding("utf-8"); //한국어,중국어,아랍어 ...전세계 모든언어가 안깨진다. 
...
//mysql insert를 진행하자, <드라이버 로드, 접속, 쿼리, 닫기>
