...
<%
//request response 객체 이용한 로그인처리 
//zmffkdldjsxmrk wjsthdgks id,pass 파라미터를 받아와 처리해보자 

//request객체를 이용하면, 클라이언트의 요청을 처리할 수 있다. 
String id=request.getParameter("id");//클라이언트가 요청시 전송한 파라미터값을 얻는 메서드 
										//매개변수로는 파라미터 변수명을 명시, 만일 html을 이용할 경우 컴포넌트에 부여된 name값을 명시
...
//드라이버로드! 
//driver jar 파일은 javaEE기반의 스팩을 따라서, 위치시켜야한다. 
//스팩에 의하면 class 파일과 jar는 WEB-INF 라는 대문자로 된 보안된 디렉토리에 위치시켜야한다. 
// 클래스의 위치는 WEB-INF/classes, jar는 WEB-INF/lib 에 둬야한다. 
